...
`pnpm lint` runs oxlint, the folder-structure budget and the lint-configuration integrity check; `docs/lint-rules.md` documents the limits, the custom `hoof/*` rules and every exemption. The authored application, tests, scripts and lint rules are linted; the starter's unchanged component catalog is outside that command. Development exposes `window.__hoof` for tests; the production build excludes it. Feature-detected WebMCP exposes only read/start/two-action controls.
