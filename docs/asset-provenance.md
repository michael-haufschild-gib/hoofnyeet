...
- The local black-hole and ghost vortex in `lib/game/effects/portal-vortex.ts` adapts polar spiral/noise techniques from Slot's `featurePortalVortexFilter` and reverses at the recorded ability release.
- `lib/game/effects/flame-mesh.ts` adapts fire-noise and warm/hot color grading from `pixi-runtime/src/shaders/anticipation/fireShaderFilter.ts`. Two locally transformed meshes keep rocket exhaust on the illustrated nozzle lips without filter render targets. Shader resources have explicit reset/disposal ownership; no independent clocks or timers were added.
