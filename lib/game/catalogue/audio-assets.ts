...
/**
 * Looping tracks: the menu theme, one per world, and the three boss acts. A
 * course keeps only the tracks it needs decoded, so these long buffers do not
 * accumulate over a tour.
 */
...
/**
 * Every sample the audio layer may request. The bundled manifest has to ship a
 * file for each id, which the asset test asserts in both directions.
 */
export type AudioId =
...
/**
 * The samples one game event plays: a propulsion event always pairs the squish
 * with the flap, otherwise the event's own sound or its alias is used. An
 * event with no bundled sample yields an empty array rather than a failed load.
 */
...
/**
 * Every id a course may need: all short effects, the menu theme, the world
 * track, and the matching boss act when `boss`. Drives both the preload and
 * the choice of which decoded music survives the level change.
 */
