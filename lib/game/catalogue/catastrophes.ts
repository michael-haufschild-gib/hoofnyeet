...
/**
 * How a trap behaves once it arrives. The driven kinds stay kinematic and are
 * stepped along a scripted path; the rest are handed to physics on release.
 */
export type Mechanism =
  ...
  | 'portal';
/**
 * One scripted disaster: `count` copies of the `prop` debris art, plus the
 * `trap` machine that strikes at `beat` seconds on the crash clock and the
 * `punchline` shown as the caption when it connects.
 */
...
/**
 * The disasters each world can stage, chosen by the run's disaster index modulo
 * four, so every world has to supply exactly four entries.
 */
