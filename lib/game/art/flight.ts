...
/**
 * Paints one lane marker: three stacked contrails around world `y`, each with a
 * chevron on its leading edge. `index` seeds the per-marker vertical wobble so
 * neighbouring markers do not line up. Alphas are 0..1.
 */
function drawLaneMarker(
  ...
  for (let row = 0; row < 3; row++) {
    const py = y - 38 + row * 38 + Math.sin(index * 1.7 + row) * 8;
    g.moveTo(x, py)
...
export function drawJetstream(
  ...
  const ribbon = proximity * (active ? 0.5 : 0.25);
  const arrow = proximity * 0.6;
  for (let i = first; i <= last; i++) {
    drawLaneMarker(g, i * spacing + drift, y, i, ribbon, arrow);
  }
}
