...
import {
  spectatorPose,
  easeSideshow as ease,
} from '../motion/spectator-motion';
import type { SceneBounds } from '../entrance-travel';

/**
 * Borrows one sprite from the owning layer's fixed pool and places it centred
 * on `x`/`y` in world units, sized to `w`/`h` pixels, rotated by `angle`
 * radians and drawn at `alpha` (0 to 1) multiplied by `tint`. Returns the
 * sprite so a caller can adjust it further, or nothing once the pool is spent.
 */
...
/** Drops a sprite back onto the track after its rotation changed. A spun prop
 * keeps the same painted foot, so its resting y has to be solved again. */
function reseat(sprite: Sprite, key: string) {
  sprite.y = -artFoot(key, sprite.width, sprite.height, sprite.rotation);
}
...
export class SpectatorShow {
  ...
  /** The bucket sprouts legs and runs off with its own brain, leaving the
   * operator flat on the track under a party hat. */
  private popcornEscape(p: Pose, bucketX: number, goose: Sprite | undefined) {
    const { x, age: t, punch } = p;
    for (const side of [-1, 1]) {
      ...
      if (leg) reseat(leg, 'skeletal-front-leg-straight');
    }
  ...
  private popcorn(p: Pose) {
    ...
    if (punch > 0) this.popcornEscape(p, bucketX, goose);
  }
  /** Five increasingly wrong photocopies fan out of the machine: every other
   * sheet is a ribcage, the rest are a single staring eye. */
  private officeCopies(p: Pose) {
    const { x, age: t } = p;
    for (let i = 0; i < 5; i++) {
      ...
      if (i % 2) {
        paper.moveTo(-8, 0).lineTo(9, 0).stroke({ color: 0x347887, width: 2 });
  ...
  /** A heart floods the helmet and orbiting bubbles fill the dome, which also
   * squashes the sheep wearing it. */
  private moonFlood(p: Pose, domeY: number, sheep: Sprite | undefined) {
  ...
  private moon(p: Pose) {
    ...
    if (k > 0) this.moonFlood(p, domeY, sheep);
    if (punch > 0) {
      for (let i = 0; i < 3; i++) {
  ...
  draw(
    ...
    const p = spectatorPose(cue, time, reduced, bounds);
    if (!p) return;
