...
/**
 * A filter that paints the displacement ability's swirl onto a transparent
 * node rather than over the scene. The caller drives `uAge` in seconds since
 * the release — arms wind in for the first second, then collapse outward —
 * `uTime` in seconds for the spin, and `uOpacity` and `uGhost` as 0..1
 * weights, `uGhost` shifting the look from black hole toward ghost.
 */
