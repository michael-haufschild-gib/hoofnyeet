...
test('explosions and late kicks carry the wreck forward with native controls, visible ground and recorded scores', async ({
  ...
  for (const at of [2.6, 5.6, 9.2]) {
    ...
    const before = await page.evaluate(() => window.__hoof.state.distance);
    ...
    expect(view.distance).toBeGreaterThan(before + 5);
  ...
  const result = await page.evaluate(() => ({
    ...
    frames: window.__hoof.recording().frames,
    rounds: window.__hoof.save.rounds,
  }));
  expect(result.flight).toBe(200);
  expect(result.distance).toBeGreaterThan(progress.at(-1)!);
  expect(result.frames.at(-1)!.distance).toBeCloseTo(result.distance, 1);
